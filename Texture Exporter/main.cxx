...
// XML
#include "3rdParty/SimpleXMLWriter.hpp"
...
int main(int p_Argc, char** p_Argv)
{
	...
	CXMLWriterNode& m_XMLMediaPack = m_XMLWriter["MediaPack"];
	...
	for (Illusion::Texture_t* m_Texture : m_Textures)
	{
		...
		if (m_GenerateConfig)
		{
			sprintf_s(m_FormatBuffer, sizeof(m_FormatBuffer), "Resource##0x%X", m_Texture->m_NameUID);
			auto& m_XMLResource = m_XMLMediaPack[m_FormatBuffer];

			// Insert output name if using hash...
			if (m_UseHashName)
				m_XMLResource("OutputName", m_Texture->m_DebugName);

			// Format (Compression)
			const char* m_Format = Helper::Texture::GetFormat(m_Texture->m_Format);
			if (m_Format)
				m_XMLResource("Compression", m_Format);

			// Path
			m_XMLResource = &m_FileName[0];
		}
	...
	if (m_GenerateConfig)
	{
		...
		m_XMLWriter.m_InsertHeader = false;
		m_XMLWriter.OutputFile(&m_ConfigPath[0]);
	}
